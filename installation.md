...
cd bitcore-wallet-service-joli
npm install
```
...
```
### Configuration
Configuration for all required modules can be specified in https://github.com/dalijolijo/bitcore-wallet-service-joli/blob/master/config.js
...
```javascript
  blockchainExplorerOpts: {
    livenet: {
      provider: 'insight',
      url: 'J_O_L_I_MAIN_INSIGHT:443',
    },
    testnet: {
      provider: 'insight',
      url: 'J_O_L_I_TEST_INSIGHT:443',
    },
  }
```
...
```javascript
  emailOpts: {
    ...
    from: 'wallet-service@jolicoinbitcore.io',
  }
```
